...
print("there are ",torch.cuda.device_count()," pieces of ",torch.cuda.get_device_name(0))
#%%
...
for times in range(0,4):
    print("-------------------------------Round {} start-------------------------------".format(times+1))
    str_org='/data/leuven/341/vsc34197/day_long/data_split/k_fold/'+'20230815_170535'+'/'+str(times)
    ...
    print(valid_list)
    print(test_list)
    model_dir = '/data/leuven/341/vsc34197/day_long/model/mst/fd_34_rest/tts/'+str_time+'/'+str(times)+'/'
    results_dir = '/data/leuven/341/vsc34197/day_long/results/mst/fd_34_rest/tts/'+str_time+'/'+str(times)+'/'
    model_dir_temp='/data/leuven/341/vsc34197/day_long/mst/fd_34_rest/tts/'+str_time+'/'    
    ...
    (x_train) = tf.keras.preprocessing.timeseries_dataset_from_array(x_train,targets=None,sequence_length=seg_length,sequence_stride=strde_length,batch_size=None,sampling_rate=DOWNSAMPLING)
    (y_train) = tf.keras.preprocessing.timeseries_dataset_from_array(y_train,targets=None,sequence_length=seg_length,sequence_stride=strde_length,batch_size=None,sampling_rate=DOWNSAMPLING) 
    Sequence_data = list()
    Sequence_label=list()
    ...
    dataLoader_train = DataLoader(dataset_total, batch_size=Batch_size,drop_last=False,shuffle=False)   
    ...
    (x_val) = tf.keras.preprocessing.timeseries_dataset_from_array(x_val,targets=None,sequence_length=seg_length,sequence_stride=strde_length,batch_size=None,sampling_rate=DOWNSAMPLING)
    (y_val) = tf.keras.preprocessing.timeseries_dataset_from_array(y_val,targets=None,sequence_length=seg_length,sequence_stride=strde_length,batch_size=None,sampling_rate=DOWNSAMPLING) 
    Sequence_data = list()
    Sequence_label=list()
    Sequence_label_class=list()
    counter=0
    ...
    dataLoader_val = DataLoader(datasetVal, batch_size=Batch_size,drop_last=False,shuffle=False)
    ...
    #创建模型
    mstcn1 = MultiStageModel(num_stages,num_layer,num_filter,num_feature,num_class) #第一个是stage个数 第二个是每个stage的层数 第三个是每层filter个数 第四个是input的channel数 第五个是类别数
    ...
    for i in range(epoch):
        ...
        for batch_ndx, sample in enumerate(dataLoader_train):
          ...
          for p in predictions:  
            ...
            loss += loss_ce(p.transpose(2, 1).contiguous().view(-1,3), oneBatchLabel2.view(-1,3))
          ...
          f1_score_counter +=1
          total += len(torch.flatten(oneBatchLabel))     #[30,2000] [batchsize,seqlength]
        ...
        with torch.no_grad():
          for batch_ndx, sample in enumerate(dataLoader_val):
            ...
            for p in predictions_val:
              ...
              loss_val += loss_ce(p.transpose(2, 1).contiguous().view(-1,3), oneBatchLabel2.view(-1,3))
            ...
            total_val += len(torch.flatten(oneBatchLabel))     #[30,2000] [batchsize,seqlength]
    ...
    for i in range(len(test_list)):
        ...
        (dataset_test) = tf.keras.preprocessing.timeseries_dataset_from_array(x_test,targets=None,sequence_length=seg_length,sequence_stride=strde_length,batch_size=None,sampling_rate=DOWNSAMPLING) 
        (target_test) = tf.keras.preprocessing.timeseries_dataset_from_array(y_test,targets=None,sequence_length=seg_length,sequence_stride=strde_length,batch_size=None,sampling_rate=DOWNSAMPLING) 
        Sequence_data = list()
        Sequence_label=list()
        ...
        with torch.no_grad():
          for batch_ndx, sample in enumerate(dataLoader_test):
            ...
            loss_val=0
            _, predicted_val = torch.max(predictions_val[-1].data, 1)   #[30,2,2000] =>max => [30,2000]
            pred_per=np.concatenate((pred_per, predicted_val.cpu().numpy()), axis=0)
            true_per=np.concatenate((true_per, oneBatchLabel.cpu().numpy()), axis=0)
            correct_val += ((predicted_val == oneBatchLabel).float().squeeze(1)).sum().item() 
          ...
          if len(conf_matrx_2)==2 and len(conf_matrx_2[0])==2:
              print(classification_report(y_true_2, y_pred_2, target_names=['0', '1']))
          else:
              print(classification_report(y_true_2, y_pred_2, target_names=['0', '1', '2']))
