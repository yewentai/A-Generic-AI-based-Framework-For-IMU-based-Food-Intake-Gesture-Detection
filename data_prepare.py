...
files = glob.glob(os.path.join(path1, "*.csv")) + glob.glob(
    os.path.join(path2, "*.csv")
)
...
grouped_files = {
    k: list(map(itemgetter(1), g)) for k, g in groupby(file_info, key=itemgetter(0))
}
...
"""
Process the mirrored data and annotations for DX-I dataset
"""

# Load all files
path1 = r"dataset/DX-I/Hand-Mirrored"
...
# Save the concatenated data to pickle files
with open("dataset/pkl_data/DX_I_X_mirrored.pkl", "wb") as f:
    pkl.dump(data_intotal, f)
with open("dataset/pkl_data/DX_I_Y_mirrored.pkl", "wb") as f:
    pkl.dump(target_intotal, f)
...
"""
Process the mirrored data and annotations for DX-II dataset
"""

# Load all files
path1 = r"dataset/DX-II/Hand-Mirrored"
files = glob.glob(os.path.join(path1, "*.csv"))
...
# Save the concatenated data to pickle files
with open("dataset/pkl_data/DX_II_X_mirrored.pkl", "wb") as f:
    pkl.dump(data_intotal, f)
with open("dataset/pkl_data/DX_II_Y_mirrored.pkl", "wb") as f:
    pkl.dump(target_intotal, f)
