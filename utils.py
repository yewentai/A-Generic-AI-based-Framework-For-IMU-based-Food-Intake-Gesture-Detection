...
# Calculate the segment-level F1 score for gesture detection
def segment_f1(y_pre_list, y_gt_list):
    FN_eat_thre = []
    FP_eat_thre = []
    data_len = len(y_pre_list)
    thre = [0.1, 0.25, 0.5, 0.75]  # Set of overlap thresholds to evaluate
    ...
    for k in range(4):
        overlap = thre[k]
        f_n_eat = np.zeros(data_len)
        f_p_eat = np.zeros(data_len)
        num_true_id_eat = np.zeros(data_len)
        ...
        for i in range(data_len):
            ...
            bg_class = 0  # Background class label
            n_classes = 2  # Number of classes, assuming binary classification here
            ...
            num_true_id_eat[i] = np.count_nonzero(true_labels == 1)  # Count true positive eating segments

            num_true_len = len(true_intervals)
            num_pred_len = len(pred_intervals)

            hit_pred_tp = np.zeros(num_pred_len)  # Hit indicator for true positives
            hit_pred = np.zeros(num_pred_len)  # Hit indicator for predictions

            # Evaluate true positive segments
            for k in range(num_true_len):
                if true_labels[k] == 1:
                    overlap_id_list = []
                    key = 0
        ...
        FN_eat_thre.append(f_n_eat)
        FP_eat_thre.append(f_p_eat)
    
    # Compute true positives and F1-score for each threshold
    TP_eat_thre = []
    F1_eat_thre = []
    total_eat_FN = []
    total_eat_FP = []
    ...
    for i in range(len(FN_eat_thre)):
        t_p_eat = np.zeros(len(FN_eat_thre[i]))
        if i == 0:
            t_p_eat = num_true_id_eat - FN_eat_thre[i]
        else:
            t_p_eat = (num_true_id_eat - FN_eat_thre[i-1]) - (FN_eat_thre[i] - FN_eat_thre[i-1]) - (FP_eat_thre[i] - FP_eat_thre[i-1])
        f1_eat = 2 * t_p_eat / (2 * t_p_eat + FN_eat_thre[i] + FP_eat_thre[i])
        TP_eat_thre.append(t_p_eat)
        F1_eat_thre.append(f1_eat)
        total_eat_FN.append(sum(FN_eat_thre[i]))
        total_eat_FP.append(sum(FP_eat_thre[i]))
        total_eat_TP.append(sum(TP_eat_thre[i]))

    total_eat_F1 = 2 * np.array(total_eat_TP) / (2 * np.array(total_eat_TP) + np.array(total_eat_FN) + np.array(total_eat_FP))

    # Evaluation of drinking gesture detection
    FN_d_thre = []
    FP_d_thre = []
    for k in range(4):
        overlap = thre[k]
        ...
        for i in range(data_len):
            ...
            n_true = true_labels.shape[0]
            n_pred = pred_labels.shape[0]
            num_true_id_drink[i] = np.count_nonzero(true_labels == 2)  # For drinking (assuming 2)

            num_true_len = len(true_intervals)
            num_pred_len = len(pred_intervals)

            hit_pred_tp = np.zeros(num_pred_len)
            hit_pred = np.zeros(num_pred_len)
            ...
            for k in range(num_pred_len):
                if pred_labels[k] == 2:
                    ...
                    start_idx, end_idx = pred_intervals[k]
                    for j in range(num_true_len):
                        if true_intervals[j][0] < end_idx and true_intervals[j][1] > start_idx and true_labels[j] == 2:
                            key += 1
                        elif j == num_true_len - 1 and key == 0:
                            f_p_drink[i] += 1
                            break
    ...
    # Print F1 scores for each overlap threshold
    for k in range(4):
        print("The threshold:", thre[k])
        print("    The eating F1 score:", total_eat_F1[k])
        print("    The drinking F1 score:", total_d_F1[k])