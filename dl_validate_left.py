...
"""
===============================================================================
MSTCN Left Hand IMU Validation Script
-------------------------------------------------------------------------------
Author      : Joseph Yep
Edited      : 2025-04-14
Description : This script validates MSTCN models on LEFT hand IMU data with:
              1. Original left hand validation
...
if __name__ == "__main__":
    ...
    versions = [d for d in os.listdir(result_root) if os.path.isdir(os.path.join(result_root, d)) and "_LEFT" in d]
    versions.sort()
    ...
    for version in versions:
        ...
        # Set up logging
        log_file = os.path.join(result_dir, "validation_left.log")
        logger = logging.getLogger(f"validation_left_{version}")
        ...
        logger.info(f"\n=== Validating Left Hand Version: {version} ===")
        ...
        # Load LEFT hand data only
        X = np.array(pickle.load(open(os.path.join(DATA_DIR, "X_L.pkl"), "rb")), dtype=object)
        Y = np.array(pickle.load(open(os.path.join(DATA_DIR, "Y_L.pkl"), "rb")), dtype=object)

        # Prepare validation modes for LEFT hand
        validation_modes = [
            {"name": "original_left", "X": X, "Y": Y},
        ]

        if mirror_enabled:
            X_mirrored = np.array([hand_mirroring(sample) for sample in X], dtype=object)
            validation_modes.append({"name": "mirrored_left", "X": X_mirrored, "Y": Y})
        ...
        if rotation_enabled:
            ...
            validation_modes.append({"name": "rotated_left", "X": X_rotated, "Y": Y})

            if mirror_enabled:
                X_rotated_mirrored = np.array([hand_mirroring(sample) for sample in X_rotated], dtype=object)
                validation_modes.append({"name": "rotated_mirrored_left", "X": X_rotated_mirrored, "Y": Y})
        ...
        # Save all statistics
        stats_file_npy = os.path.join(result_dir, "validation_left_stats.npy")
        stats_file_json = os.path.join(result_dir, "validation_left_stats.json")
        np.save(stats_file_npy, all_stats)
        with open(stats_file_json, "w") as f_json:
            json.dump(all_stats, f_json, indent=4)
        logger.info(f"\nLeft hand validation statistics saved to {stats_file_npy} and {stats_file_json}")
